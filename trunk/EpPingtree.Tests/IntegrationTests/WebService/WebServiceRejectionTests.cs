...
{
    [TestFixture]
    public class WebServiceRejectionTests : BaseTestWithTestClass<ApplyLeadService>
    {
        ...
        public void TestRejectedLeads()
        {
            using (BeginScope())
            {
                ...
                Assert.AreEqual(3, leadRejecteds.Count);
                ...
                int buyer3Id = leadRejecteds[2].BuyerId;

                Buyer buyer3ToCompare = _buyersToDelete.Where(a => a.BuyerId == buyer3Id).FirstOrDefault();
                Assert.AreEqual("FAKE1", buyer3ToCompare.Name);

                int[] buyerInts = new int[]{buyer1Id,buyer2Id,buyer3Id};

                LeadBought orderSuccess = (from leadBought in context.LeadBoughts
                                             where buyerInts.Contains(leadBought.BuyerId)
                                             select leadBought).FirstOrDefault();

                Assert.IsNull(orderSuccess);

                Assert.AreEqual(3, sellLeadResponse.ErrorMessage.ErrorReasons.Count());

                Assert.AreEqual(ESellLeadResponse.Rejected, sellLeadResponse.Result);
            }
        }
